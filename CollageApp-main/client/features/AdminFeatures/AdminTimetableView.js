...
//   }, [],);
  ...
  const renderTabView = ({ navigationState }) => {
    ...
    const dayEvents = events.filter(
      (event) => moment(event.startTime).format("dddd") === moment().format("dddd")
    );

    const dayTimetable = timetable.filter(
      (timetable) => moment(timetable.startTime).format("dddd") === moment().format("dddd")
    );

    const dayTeachers = teachers.filter(
      (teacher) =>
        moment(teacher.timetable.startTime).format("dddd") === moment().format("dddd")
    );
        ...
        <View style={styles.eventNotification}>
          {dayEvents.length > 0 &&dayEvents.map((event) => (
              <View key={event.id}>
                ...
                {event.isHoliday ? (
                  <Text style={styles.eventDescription}>Institution is closed</Text>
                ) : (
                  <Text style={styles.eventDescription}>Regular classes will be held</Text>
                )}
              </View>
            ))}
        </View>
